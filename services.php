<?php 
if($_GET['submodule'] == "list")
{
?>

  <section id="list" >
      <div class="container">

      <div class="section-title">         
        <p>List of Services</p>
        <hr class="title-underline">
      </div>
        
   

      <div class="row content">
      
        <div class="col-lg-12 ">
          <p>
          We bring together a team of expertise in providing total solutions that ensures client's success and satisfaction.
          </p>
          <div class="row"> 
            <div class="col-lg-2">
            <img src="assets/service1.png" class="services_img" >
            </div>
            <div class="col-lg-10">
              <p class="mt-4"><span class="yellow">SELL AND PURCHASE OF USED INDUSTRIAL MACHINERIES, EQUIPMENT AND TOOLS</span></p>
              <p>Our collection system enables us to purchase metal scrap at high rates.</p>
            </div>
            <div class="col-lg-2"></br>
            <img src="assets/service2.png" class="services_img"  >
            </div>
            <div class="col-lg-10"></br>
              <p class="mt-4"><span class="yellow">DISMANTLEMENT AND DECONSTRUCTION OF INDUSTRIAL MACHINERIES AND EQUIPMENT</span></p>
              <p>Our experienced workforce allows us to provide high quality service with efficiency.</p>
            </div>
            <div class="col-lg-2"></br>
            <img src="assets/service3.png" class="services_img" >
            </div>
            <div class="col-lg-10"></br>
              <p class="mt-4"><span class="yellow">SELLING OF RAW MATERIALS FOR STEEL AND CASTING.</span></p>
              <p>We are experts in manufacturing materials such as used iron and pressed iron scrap for casting plants, in addition to iron for electric furnaces.</p>
            </div>
            <div class="col-lg-2"></br>
            <img src="assets/service4.png" class="services_img">
            </div>
            <div class="col-lg-10"></br>
              <p class="mt-4"><span class="yellow">SELL AND SUPPLY OF RIVER SAND.</span></p>
              <p>We provide our customer with best selling price and quality for river sand used for construction.</p>
            </div>           
          </div>
          
        </div>
        <div class="col-lg-12">
          <hr class="gold-line mt-4 ">        
          <p class="mt-4">We bring together a team of expertise in providing total solutions that ensures client's success and satisfaction.</p>
          <p>
        </div>
        <div class="col-lg-7">
            <span class="yellow">INDUSTRIAL WASTE PROCESSING</span>
          </p>
          <ul>
            <li class="text-justify"><i class="ri-check-double-line"></i> We are licensed to collect and transport industrial waste in every region of Malaysia, and engage in a wide range of processing with the cooperation of regional disposal contractors.
            </li>
            <li class="text-justify"><i class="ri-check-double-line"></i> A wide range of facilities and equipment deconstruction, Including furnaces, processing machinery, presses, welding machines, coating equipment, conveyors, boilers, analysis and deconstruction of small PCB equipment, and asbestos analysis and removal.
            </li>
            <li class="text-justify"><i class="ri-check-double-line"></i> Deconstruction of steel, reinforced concrete, and wood structures, as well as smokestacks.
            </li>
            <li class="text-justify"><i class="ri-check-double-line"></i> E-waste recycling services, Office Dismantling Services, Building Deconstruction services, Machinery scrap and all Kinds of Cable Wire
            </li>
          </ul>

        </div>
        <div class="col-lg-5">
        <img src="assets/waste_process.png" class="waste_process">
        </div>

      </div>

      </div>
  </section>

<?php 
}
elseif($_GET['submodule'] == "material")
{
?>

  <section id="material" >
      <div class="container">

      <div class="section-title">         
        <p>Type of collected Material </p>
        <hr class="title-underline">
      </div>
      
   

      <div class="row content ">
      
      <div class="col-lg-6">
        <div class="text-center mb-4">
          <span class="yellow tex">FERROUS MATERIALS</span>  
        </div>
        
        <img src="assets/ferrous_materials.png" class="waste_process">
        </div>
        <div class="col-lg-6">
          <div class="text-center  mb-4">
            <span class="yellow">NON-FERROUS MATERIALS</span> 
          </div>
        
        <img src="assets/nonferrous_materials.png" class="waste_process">
        </div>

        <div class="text-center mt-4">
          <br>
          Among collected non-ferrous metals including aluminium, nickel, lead, tin, brass, silver, and zinc
        </div>
        </div>

      </div>
  </section>

<?php 
}
elseif($_GET['submodule'] == "safety")
{
?>

  <section id="safety" >
      <div class="container">

      <div class="section-title">         
        <p>Safety is our main priority </p>
        <hr class="title-underline">
      </div>
        
   

      <div class="row content">
      <p class="mb-4">Prior to any project implementation, we make sure our team is fully trained and certified for Safety and Health.</p>
      <div class="col-lg-4 ">
        <img src="assets/safety1.png" class="waste_process">
        </div>
        <div class="col-lg-4 ">
        <img src="assets/safety2.png" class="waste_process">
        </div>
        <div class="col-lg-4 ">
        <img src="assets/safety3.png" class="waste_process">
        </div>

        </div>

      </div>
  </section>

<?php 
}
elseif($_GET['submodule'] == "safetyequipment")
{
?>

  <section id="safetyequipment" >
      <div class="container">

      <div class="section-title mb-4">         
        <p>Safety Equipments </p>
        <hr class="title-underline">
      </div>
        
   

      <div class="row content">      
      <div class="col-lg-2 ">        
            <img src="assets/firstaid.png" class="services_img">          
            <p class="text-center mt-4">FIRST AID KIT</p>
      </div>
      <div class="col-lg-2 ">        
            <img src="assets/safetybelts.png" class="services_img">          
            <p class="text-center mt-4">SAFETY BELTS</p>
      </div>
      <div class="col-lg-2 ">        
            <img src="assets/safetywest.png" class="services_img">          
            <p class="text-center mt-4">SAFETY VEST</p>
      </div>
      <div class="col-lg-2 ">        
            <img src="assets/glove.png" class="services_img">          
            <p class="text-center mt-4">HAND GLOVE</p>
      </div>
      <div class="col-lg-2 ">        
            <img src="assets/helmet.png" class="services_img">          
            <p class="text-center mt-4">SAFETY HELMET</p>
      </div>
      <div class="col-lg-2 ">        
            <img src="assets/shoe.png" class="services_img">          
            <p class="text-center mt-4">SAFETY SHOE</p>
      </div>
        </div>
        <div class="row content mt-4">      
      <div class="col-lg-2 ">        
            <img src="assets/eye.png" class="services_img">          
            <p class="text-center mt-4">EYE WEAR</p>
      </div>
      <div class="col-lg-2 ">        
            <img src="assets/oxygen.png" class="services_img">          
            <p class="text-center mt-4">OXYGEN AND FITTINGS</p>
      </div>
      <div class="col-lg-2 ">        
            <img src="assets/mask.png" class="services_img">          
            <p class="text-center mt-4">BREATH MASK</p>
      </div>
      <div class="col-lg-2 ">        
            <img src="assets/fireextinguisher.png" class="services_img">          
            <p class="text-center mt-4">FIRE EXTINGUISHER</p>
      </div>
      <div class="col-lg-2 ">        
            <img src="assets/ear.png" class="services_img">          
            <p class="text-center mt-4">EAR SAFETY</p>
      </div>
      <div class="col-lg-2 ">        
            <img src="assets/cone.png" class="services_img">          
            <p class="text-center mt-4">CONES</p>
      </div>
        </div>

      </div>
  </section>

<?php 
}
elseif($_GET['submodule'] == "equipment")
{
?>

  <section id="material" >
      <div class="container">

      <div class="section-title mb-4">         
        <p>Company Equipments </p>
        <hr class="title-underline">
      </div>
        
   

      <div class="row content">
      
      <div class="col-lg-4 ">
        <img src="assets/equipment1.png" class="equipment_img">          
            <p class="text-center yellow mt-4">MAGNETIC EXCAVATOR</p>
        </div>
        <div class="col-lg-4 ">
        <img src="assets/equipment2.png" class="equipment_img">          
            <p class="text-center yellow mt-4">FORKLIFT (3 TONNE)</p>
        </div>
        <div class="col-lg-4 ">
        <img src="assets/equipment3.png" class="equipment_img">          
            <p class="text-center yellow mt-4">EX300 EXCAVATOR</p>
        </div>

        </div>
        <div class="row content">
      
      <div class="col-lg-6">
        <img src="assets/equipment4.png" class="equipment_img">          
            <p class="text-center yellow mt-4">HYDRAULIC BREAKER</p>
        </div>
        <div class="col-lg-6 ">
        <img src="assets/equipment5.png" class="equipment_img">          
            <p class="text-center yellow mt-4">JCB BACKHOE</p>
        </div>
       

        </div>

      </div>
  </section>
<?php 
}
?>